import { useDeepCompareEffect } from '@licitame/hooks';
import LicitameLayouts from '@licitame/layouts/LicitameLayouts';
import _ from '@lodash';
import { makeStyles } from '@material-ui/core/styles';
import AppContext from 'app/AppContext';
import * as Actions from 'app/store/actions';
import { generateSettings } from 'app/store/reducers/licitame/settings.reducer';
...
function LicitameLayout(props) {
	const dispatch = useDispatch();
	const settings = useSelector(({ licitame }) => licitame.settings.current);
	const defaultSettings = useSelector(({ licitame }) => licitame.settings.defaults);
	...
	const Layout = useMemo(() => LicitameLayouts[settings.layout.style], [settings.layout.style]);

	return _.isEqual(newSettings.current, settings) ? <Layout classes={{ root: classes.root }} {...props} /> : null;
}

export default React.memo(LicitameLayout);
